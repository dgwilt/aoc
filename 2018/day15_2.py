...
def do_attack(foes,players,xdim,world,recently_deceased,armies,ap,myrace):
    ...
    if players[attack]['hp'] <= 0:
        ...
        armies[dead_race] -= 1
        return (armies[dead_race] == 0, dead_race is E)
    else:
        return False, False
...
def run_battle(data,elf_power):
    ...
    while True:
        ...
        for pnum,mypos in enumerate(read_order(xdim,players.keys())):
            ...
            if foes:
                finished, dead_elf = do_attack(foes,players,xdim,world,recently_deceased,armies,ap,myrace)
                if dead_elf:
                    return -1
                elif finished:
...
def run(data):
    elf_power = 4
    while True:
        outcome = run_battle(data,elf_power)
        if outcome < 0:
            elf_power += 1
        else:
            return outcome
