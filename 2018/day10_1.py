...
data = """position=< 11153,  22033> velocity=<-1, -2>
...
class Particle:
    ...
    def move(self,t=1):
        self._p = [self._p[i]+(self._v[i]*t) for i in range (2)]

def corners(particles):
    xmin = min(particles, key=lambda p: p.x()).x()
    xmax = max(particles, key=lambda p: p.x()).x()
    ymin = min(particles, key=lambda p: p.y()).y()
    ymax = max(particles, key=lambda p: p.y()).y()
    return xmin,xmax,ymin,ymax

def run(data):
    particles = [Particle(*[int(i) for i in search(r'position=<\s*([-\d]+),\s*([-\d]+)> velocity=<\s*([-\d]+),\s*([-\d]+)>',l).group(1,2,3,4)]) for l in data.splitlines()]

    for tdelta in [100,1]: # Go forward in time jumps of 100 until we find the rough area, then go down to 1
        last_area = None
        while True:
            xmin,xmax,ymin,ymax = corners(particles)
            area = (xmax - xmin) * (ymax - ymin)

            if last_area is not None and area > last_area:
                # Rewind one time delta to before the area grew
                for p in particles: p.move(-tdelta)
                if tdelta != 1: break
                sky = set(p.pos() for p in particles)
                xmin,xmax,ymin,ymax = corners(particles)
                return "\n".join(["".join("#" if (x,y) in sky else " " for x in range(xmin, xmax+1)) for y in range(ymin, ymax+1)])
