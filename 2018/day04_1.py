...
data = """[1518-05-29 00:00] Guard #1151 begins shift
...
tests = ["""[1518-11-01 00:00] Guard #10 begins shift
[1518-11-01 00:05] falls asleep
...
def run(data):
    ...
    guardlog = defaultdict(lambda:[0]*60)
    for line in sorted(data.splitlines()):
        t,report = search(r'\[\d+-\d+-\d+ \d+:(\d+)\]\s+(.*)',line).group(1,2)
        if report.startswith("falls"):
            start = int(t)
        elif report.startswith("wakes"):
            for m in range(start, int(t)):
                guardlog[guard][m] += 1
        elif report.startswith("Guard"):
            guard = int(search(r'Guard #(\d+) begins shift',report).group(1))

    times = {sum(s):g for g,s in guardlog.items()}
    sleepiest = times[max(times.keys())]
    sleeptimes = guardlog[sleepiest]
    best_minute = sleeptimes.index(max(sleeptimes))

    return best_minute * sleepiest
