data = """fjkfpm (69) -> kohxzh, liwvq, eqkio, xvoyybs
dsiixv (52)
fhimhm (66)
mdlubuq (73)
ulobwyb (41)
...
def weigh(node,tower,weights):
	return weights[node] + sum([weigh(d,tower,weights) for d in tower[node]])
...
def run(data):
	...
	for line in data.split("\n"):
		holding,weight,*held = line.replace(",","").split()
		if len(held) > 0:
			held.pop(0) # get rid of the arrow
			tower[holding] = held
			weights[holding] = int(weight[1:-1]) # get rid of the brackets
			all_held.update(held)
		else:
			tower[holding] = []
			weights[holding] = int(weight[1:-1])

	# start at the root
	node = (set(tower.keys()) - all_held).pop()
	weightdif = None
	while True:
		branch_weights = [weigh(b,tower,weights) for b in tower[node]]
		dif = max(branch_weights) - min(branch_weights)
		if dif == 0:
			# Found the balanced node
			return weights[node] - weightdif
		else:
			if weightdif is None:
				# Grab the weight difference at the start
				weightdif = dif
			# Go to the heaviest node and look again
			node = tower[node][branch_weights.index(max(branch_weights))] # Assumes that the node needs to lose weight, not gain it.
