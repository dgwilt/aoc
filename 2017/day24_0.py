...
def run(data):
	...
	while bridges:
		...
		for bridge, start in zip(bridges, ends): # The end of the bridge is the start of the next piece
			available_parts = all_parts - bridge # Use set operator to only use the pieces that are not in the current bridge
	...
	return max([sum([sum(part) for part in bridge]) for bridge in finishedbridges])
