...
from aoc import AocDay, sortstr
...
class AocDay8(AocDay):
    ...
    def run_silver(self,data):
        result = 0
        for _, digits in [line.split(" | ") for line in data.splitlines()]:
            result += sum(len(d) in (2,3,4,7) for d in digits.split())
        return result
    
    @staticmethod
    def deduce(pats):
        # Helper functions
        filterlen = lambda n: [p for p in pats if len(p) == n]
        difference = lambda a,b: [l for l in a if l not in b]
        fivesegs =  filterlen(5)
        sixsegs = filterlen(6)

        # The unique length digits
        one = filterlen(2)[0]
        seven = filterlen(3)[0]
        four = filterlen(4)[0]
        eight = filterlen(7)[0]
        ...
        # The rest!
        bd = difference(four,one)
        zero  = [p for p in sixsegs  if any(l not in p for l in bd)][0]
        nine  = [p for p in sixsegs  if all(l in p for l in four)][0]
        six   = [p for p in sixsegs  if any(l not in p for l in one)][0]
        three = [p for p in fivesegs if all(l in p for l in one)][0]

        fivesegs.remove(three)

        e = difference(eight,nine)[0]
        five = [p for p in fivesegs if e not in p][0]

        d = difference(eight,zero)[0]
        b = [l for l in bd if l != d][0]
        two = [p for p in fivesegs if b not in p][0]
        
        digits = [zero,one,two,three,four,five,six,seven,eight,nine]
        return {sortstr(d):str(i) for i,d in enumerate(digits)}

    def run_gold(self,data):
        result = 0
        for pats, digits in [line.split(" | ") for line in data.splitlines()]:
            segments_to_digit = AocDay8.deduce(pats.split())
            result += int("".join(segments_to_digit[sortstr(d)] for d in digits.split()))
        return result

if __name__ == "__main__":

    data = """cbgefad agc fdega cgdf ecdgfa efgca gaefbd edagbc cg ecafb | cdgafbe cfdg cg gac
...
febcga bfgcda afc cbagefd agcd bfecgd ac dfbca cbfgd befad | ac fca afc cdfab"""
    ...
    silver_tests = ["""be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
