...
def force(data,x,y):
    return Intcode().program(data).provide_input(x,y).execute().pop_outputs()[0]
...
def run(data):
    ...
    pull = "#"
    affected = 0
    grid = [[None for _ in range(xdim)] for _ in range(ydim)]
    for x,y in product(range(xdim),range(ydim)):
        beam = force(data,x,y)
        affected += beam
        if beam: grid[y][x] = pull
        
    # Bottom edge ratio used for part 2
    edge = []
    for x,y in product(range(1,xdim),range(ydim)):
        if grid[y][x] == pull and grid[y][x-1] != pull:
            edge.append((x,y))
    
    print(sum(x/y for x,y in edge)/len(edge))
