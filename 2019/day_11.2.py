...
deltas = {'S':(0,1),'N':(0,-1),'W':(-1,0),'E':(1,0)}
left = 0
right = 1
turns = {
    ('S',right):'W',
...
def run(data):
    ...
    panels[pos] = white
    while True:
        colour = panels.get(pos,black)
        computer.provide_input(colour).execute()
        if computer.status == Intcode.Finished:
            break
        paint,turn = computer.pop_outputs()
        panels[pos] = paint
        dir = turns[(dir,turn)]
        delta = deltas[dir]
        pos = tuple(pos[i] + delta[i] for i in range (2))
    
    x,y = 0,1

    mins = tuple(min(panels.keys(),key=lambda p:p[i])[i] for i in [x,y])
    maxs = tuple(max(panels.keys(),key=lambda p:p[i])[i] for i in [x,y])

    xdim = maxs[x] - mins[x] + 1
    ydim = maxs[y] - mins[y] + 1
    grid = [[" " for _ in range(xdim)] for _ in range(ydim)]
    palette = {black:" ", white:"█"}
    for pos,colour in panels.items():
        xp,yp = tuple(pos[i] - mins[i] for i in [x,y])
        grid[yp][xp] = palette[colour]
