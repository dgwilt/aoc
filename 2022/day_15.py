...
class AocDay15(AocDay):
    ...
    def parse_sensors(self,data):
        ...
        for line in data.splitlines():
            sx,sy,bx,by = [int(i) for i in search(r'Sensor at x=([\d-]+), y=([\d-]+): closest beacon is at x=([\d-]+), y=([\d-]+)',line).groups()]
            mdist = abs(sx-bx) + abs(sy-by)
            sensors.append((sx,sy,mdist))
            beacons.add((bx,by))
            xmin = min(xmin,sx - mdist)
            xmax = max(xmax,sx + mdist)

        return sensors, beacons, xmin, xmax

    def run_silver(self,data):
        y,_,data = data
        sensors,beacons,xmin,xmax = self.parse_sensors(data)
        return sum(any(abs(x-sx) + abs(y-sy) <= dist for sx,sy,dist in sensors) and (x,y) not in beacons for x in range(xmin,xmax+1))
        
    def run_gold(self,data):
        _,MAX,data = data
        sensors,_,_,_ = self.parse_sensors(data)
        for y in range(MAX,-1,-1): # Reverse search assuming in second half of world
            x = 0
            while x <= MAX:
                xo = x
...
if __name__ == "__main__":
    ...
    silver_tests = [(10,20,"""Sensor at x=2, y=18: closest beacon is at x=-2, y=15
...
Sensor at x=20, y=1: closest beacon is at x=15, y=3""")]
    ...
    data = (2000000,4000000,"""Sensor at x=3889276, y=3176133: closest beacon is at x=3738780, y=3090050
