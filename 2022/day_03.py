...
class AocDay3(AocDay):
    ...
    PRIORITY = {l:c for c,l in enumerate(ascii_lowercase + ascii_uppercase, start=1)}

    def run_silver(self,data):
        total = 0
        for rucksack in data.splitlines():
            l = len(rucksack)//2
            one, two = set(rucksack[:l]), set(rucksack[l:])
            total += AocDay3.PRIORITY[one.intersection(two).pop()]
        return total
            
    def run_gold(self,data):
        rucksacks = [set(l) for l in data.splitlines()]
        groups = [rucksacks[i:i+3] for i in range(0,len(rucksacks),3)]
        total = sum(AocDay3.PRIORITY[set.intersection(*g).pop()] for g in groups)
        return total

if __name__ == "__main__":

    silver_tests = ["""vJrwpWtwJgWrhcsFMMfFFhFp
...
CrZsJsPPZsGzwwsLwLmpwMDw""",""""""]
    ...
    data = """VdzVHmNpdVmBBCpmQLTNfTtMhMJnhFhTTf
