#!/usr/bin/env python3 
from sys import argv
from aoc import AocDay
from re import findall

class AocDay22(AocDay):

    def parse_input(self,data):
        EMPTY = " "
        map, instructions = data.split("\n\n")
        instructions = findall(r'(\d+)([LR]?)',instructions)
        grid = {}
        for y,row in enumerate(map.splitlines()):
            for x,cell in enumerate(row):
                if cell != EMPTY: grid[(x,y)] = cell
        return grid, instructions

    def password(self,pos,dir):
        x,y = pos
        return 1000*(y+1) + 4*(x+1) + "RDLU".index(dir)

    def move_forwards(self,pos,dir):
        delta = self.udlr[dir]
        return (pos[0] + delta[0], pos[1] + delta[1]), dir

    def move_silver(self,grid,pos,dir):
        R,L,U,D = "R","L","U","D"
        nextpos, nextdir = self.move_forwards(pos,dir)
        if nextpos not in grid:
            if dir == R  : nextpos = min(p for p in grid.keys() if p[1] == pos[1])
            elif dir == L: nextpos = max(p for p in grid.keys() if p[1] == pos[1])
            elif dir == D: nextpos = min(p for p in grid.keys() if p[0] == pos[0])
            elif dir == U: nextpos = max(p for p in grid.keys() if p[0] == pos[0])
        return nextpos,nextdir

    def move_gold(self,grid,pos,dir):
        # NB does not work for the test data, just the real data, as the nets are different
        SIDE = 50
        R,L,U,D = "R","L","U","D"
        nextpos, nextdir = self.move_forwards(pos,dir)
        if nextpos not in grid:
            x,y = pos
            if x >= 2*SIDE:
                # Right cube side
                if dir == U:
                    nextdir = U
                    nextpos = (x-2*SIDE, 4*SIDE-1)
                elif dir == R:
                    nextdir = L
                    nextpos = (2*SIDE-1, 2*SIDE+(SIDE-y)-1)
                elif dir == D:
                    nextdir = L
                    nextpos = (2*SIDE-1, SIDE + x - (2*SIDE))

            elif x < SIDE:
                if y >= 3*SIDE:
                    # Back cube side
                    if dir == D:
                        nextdir = D
                        nextpos = (2*SIDE + x,0)
                    elif dir == L:
                        nextdir = D
                        nextpos = (SIDE+y-(3*SIDE), 0)
                    elif dir == R:
                        nextdir = U
                        nextpos = (SIDE+y-(3*SIDE), 3*SIDE-1)

                else:
                    # Left cube side
                    if dir == L:
                        nextdir = R
                        nextpos = (SIDE,SIDE-(y-2*SIDE)-1)
                    elif dir == U:
                        nextdir = R
                        nextpos = (SIDE,SIDE+x)

            else:
                if y < SIDE:
                    # Top cube side
                    if dir == U:
                        nextdir = R
                        nextpos = (0, 3*SIDE + x-SIDE)
                    elif dir == L:
                        nextdir = R
                        nextpos = (0, 2*SIDE + SIDE-y-1)
                elif y < 2*SIDE:
                    # Front cube side
                    if dir == L:
                        nextdir = D
                        nextpos = (y-SIDE,2*SIDE)
                    elif dir == R:
                        nextdir = U
                        nextpos = (2*SIDE+y-SIDE,SIDE-1)

                else:
                    # Underneath cube side
                    if dir == R:
                        nextdir = L
                        nextpos = (3*SIDE-1, SIDE-(y-2*SIDE)-1)
                    elif dir == D:
                        nextdir = L
                        nextpos = (SIDE-1, 3*SIDE + x-SIDE)
        return nextpos,nextdir

    def run_sim(self,data,move_fn):
        R,L,U,D,WALL = "R","L","U","D","#"
        TURNS = {R:{L:U,R:D},L:{L:D,R:U},D:{L:R,R:L},U:{L:L,R:R}}

        grid, instructions = self.parse_input(data)
        pos, dir = min(grid.keys(),key=lambda p:(p[1],p[0])), R

        for move,turn in instructions:
            for _ in range(int(move)):
                nextpos,nextdir = move_fn(grid,pos,dir)
                if grid[nextpos] == WALL: break
                pos, dir = nextpos, nextdir

            if turn: dir = TURNS[dir][turn]

        return self.password(pos,dir)

    def run_silver(self,data):
        return self.run_sim(data,self.move_silver)

    def run_gold(self,data):
        return self.run_sim(data,self.move_gold)

if __name__ == "__main__":

    silver_tests = ["""        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5""",""""""]

    gold_tests = ["""""",""""""]

    data = """                                                  ............#..#..................#............#.......#..............#.............................
                                                  ...#...#...#.......#............................#.#.....#..#...........................#......#.....
                                                  #.....#....#....#...............#..#.........#..................#......................#.##......#..
                                                  ...#........#.....#...........#.......#.......###....#.......#.............##...........#.#.........
                                                  ..............#.......#............................#.....#..........#....#.........................#
                                                  ...................#.#........#.......#.......#.........#......................#......#.#...#.....#.
                                                  ......#.#......#....#...........................#..........##..............#................#..#....
                                                  ....#...#..#...............................................................#.............#..........
                                                  .###.#........#.................#............#....................#....................#............
                                                  .........................#....#............#.....#.......#......#........#.....#..........#.........
                                                  ..............#....#.........#...#...#...........#..............#....##................#........#...
                                                  ..............#......#.#...................#.....................................#......#....#......
                                                  ..#......#.#................#........#...........#...#...#....#...............#.....#...............
                                                  .#..#...##................#......#..........................#.#....................#...............#
                                                  ......................#..#............#..#...............#..............##........#..............#..
                                                  ......#..#........#.......#..........#.........#...#.....#............##...#.##...#...............#.
                                                  .......#....#..#.............#.........#...........#........#........#.........#..#...........#..#.#
                                                  ...................#............#...............#.....#.............#.#......................#......
                                                  ...#..........#.................#..........#..#.................#.......#..#....#...#...............
                                                  ...#..##...#.....#.....................##...............#.........##.#.......###......###.#.#.##....
                                                  .............#.....#..............#...............#..........#.............................##.......
                                                  ...#......#....#.............................#..................#.#....#...............#....#.......
                                                  ....#..................#................#......#..................................#..##...#.........
                                                  ...#.......#............##...#..#...#...##.....#..#....................................#............
                                                  .#.....#...#..........................#.........................................#.#......#........#.
                                                  .........#.......................###..........#..............................#......#..##...........
                                                  ..................#.#...#.#..............##...................#........#....................#.#.....
                                                  ........#...........................#.........#.....#...#......................#..........#.........
                                                  ..........#........#........#.........................#................#.................#......#...
                                                  .......................#.#.........................#.....#....#......#.....#..#......#..............
                                                  ....###.........#......#..........#...............#......#...........#.....#....#.......#...........
                                                  ................#.#...#............#..#.........#..................#...................#...#........
                                                  ....#........#.............#..........................#...#......#.....#.#......#..............###..
                                                  ...#...........................#..#...................##.....##......#.#.......#..............#.....
                                                  ...#.............#...#..................#........#........#......#...#..................#....##.#...
                                                  .#...#.......##......#.....#................#..#.....#...........#.......#..........................
                                                  ..............#......##......................#.#.......#............................................
                                                  ...............................#........#..........................................#......#.........
                                                  ..#..#...........#..#.......#.#..........#......#.........#...#..................#.#.......#..#.#...
                                                  ........#......##.#......#.........#...#.........#................#...............#................#
                                                  ....#..........#..........................#..#................................#......#....#.........
                                                  ...#.............##........#......#.........#...................................#.......#......#.#..
                                                  ..#....#......##.#.............#................#......#....................................#......#
                                                  ...............................#........#.....#.#.............#.#......#........#.............##....
                                                  .......#..........#.........#..#..........#.............#.............................#..#..........
                                                  ......#....#...#.##.................#...............##..........................#................#..
                                                  ....#...........................#.....#....................#.......#.....................#..........
                                                  .....####........#.....#...#.....#.#.#.....#............................#.....................#.....
                                                  .....................##..................#..........#.........##......#....................#....#...
                                                  .......................#...........................................#......#.#.......................
                                                  ....#........#...................................#
                                                  ...#........#............................#..#...##
                                                  #.....#.............#..........#.......##...#..##.
                                                  .#.#.#..#......................#........#.....#...
                                                  ........................#..............#..........
                                                  .............................#................#...
                                                  .#........#.....................##.#.#.....#...#..
                                                  .....#.#....#.........#....#..............##...##.
                                                  .#...................#...#..........##............
                                                  ....#........##.......#.................#.....##..
                                                  ....#....##.....#...#..#..##....................#.
                                                  .............................##......#............
                                                  ..............#.................#.............#...
                                                  .......#...#...........#..#....#........#...##....
                                                  .......................#......#...........#...#...
                                                  ........................#.........#.............#.
                                                  .....#...........#................................
                                                  ....#..........#.#....#..#........................
                                                  ......#.....#..##.....###.................#.......
                                                  .........................#........#.......#.....#.
                                                  ........###..##.##.........#...........##....#....
                                                  ....#...#................##.#..........#.#........
                                                  ...........#...........#..#...#.......###.........
                                                  .........................#.#...#.....#....#.#.....
                                                  ..##..#............#...........#.....#.#..........
                                                  ..#...........#..#...........#...#.......#.....#..
                                                  #.......#.........#.....#.....#......#....#..#....
                                                  ..#..#.....#.#.................................#..
                                                  .............#.................#..#...............
                                                  ....#..............#............#.................
                                                  ..#.#......###..........##........#...............
                                                  .......#.............#...#........................
                                                  .#.......#..........................#.....##......
                                                  ...........#.....#...#.............#..............
                                                  #...........#.................##......#..##.......
                                                  .##....#..............#........#...........#......
                                                  .#...........#.#...#..#.......#.....#.............
                                                  .........#............#...............#...........
                                                  ....#.....................#..................#....
                                                  .#........#......#...........................#..#.
                                                  ..................#...#..........#...............#
                                                  .....................#..................#.........
                                                  ............#.....#.....#.........................
                                                  .........#........................##.............#
                                                  .#..#...........##.........#.....#....#.....#.....
                                                  .................................#....##..........
                                                  ..##.............#.....#..#...#...........#...#...
                                                  ..............................#.#.........#.......
                                                  ..................#......#..#.....................
                                                  .........#.........#...#.....#..#...........#....#
#.........................#......#...........#..##.#................#.................#...#.........
.................#............................#..............#...#..#.........#....#.......#.#.#....
##................##.......................#......................#.........#.......................
.........#......................#...........#...#......#..#........#...#....##........#.#......#....
..............#.......#........#..#...#........#........#..#........#.............#.#.#......#...#.#
.#.....#...#..............#.#.............#.#...#.....................#....#............#...........
...#................#..........#.............##.........................#.........#..#.........##...
.....................................##.#...#.#................#.................#..................
...........................#........#...................#.............#......#............#.#.......
....#.................#....#..#......................#.............#................................
........##.....#......#.#...........##................#....................#........#......#.....##.
#...................#.................................#....................#...#......#....#..##....
...................#....##..#.................#........#.........##.....................#...#....#..
..........#...#..............#.......#................#...........#..........#..#..#..#.........#...
.............#..#....#.....#......#...........#...........#........#.......#..#.#.##.....##......#..
....#.....#..#..............#...............#.......#.....................#.................#.......
...........#..........................#.........#.#....#.............................#.....#........
..............#........#....#.........#........#..#........#....#................#................#.
.....#............#.............#.........................................#.....##....#..........#..
.........#..........#...............................#..#...........#...#....#.................#.....
..#...........#.....................#....#..........................#.........#.....................
..............#.#....#.......#.....#..#.................#............#......#..................#.#..
...#...............#........#............#....#........#..........##.#....##...........##...........
.....#......#............#.........#.........#............#..............................#..........
...........#.#........#....#.....#.............................................#....................
........#......#..............................#.#...................#.#...........#.#...............
.#...#............#.......#......#...#.#...#...#....#........#................##..#.................
...........................#..........................#.....#..................#.........#..........
#..#..............#.#...............#..........##..........##......................#....#...........
.#................#......#.................#..............#.................#.......................
............#............#...#.........#.#.............................#...................#.....#..
.......#.............#.....#..................#.......#..............#..............................
.....#.......#.#.#..#.....#.....#.#.#.......#.#.....##..........................#...................
......##.......##.....#....#........................#..#...........#............................#...
........#..##..#................#.............#..........#..#.....................#..............#..
........#..........#..................##............................#......#..................#...#.
............#..................#...........#....#....#.............#.....................#...#..#...
....#.............##.#.........#.#......#.#.##..............#.....#......#.............#....#......#
......#.............#.......#.....................#.....#......#..#.................................
...#................#....#.#............................#...#..#...........#............#..#........
......#...................#...#......#........#.........#.........#..........#......#...#.......#...
.#.....#...................#.......#.....##......#................##.....##.#......#................
...#............#.........................#...................#...#..........#.#...#..#.#.......#...
......##............#.#............#..#............#.......................#........#.#...#....#..#.
.............................................................#.....................#....#.##........
..#................#..#....#....#.........................#.#.....#......................#....#.#...
.........................#...#.....................#.........#..........#.#......#................#.
..#.#...#........#....................##.............#...............#.....#.................#......
........#.......#.#...........#.................#....#..............#..........#.#.#................
........#.#.###............##............#........#...........................#...#.................
........................................#.........
..........##.#......#.............................
..............#........#.....#.#.#..............#.
....#...........#.#.#................#..#.......#.
..#...#.##...........#............................
..#.....#..#............#....................#....
.........#....#................................#..
............#.......#.......#...............#.....
............................#...#......##.........
......#.#...............#.........................
...............#...............#............#.#...
...#.#.........#....#.....#......#................
......#.....#.....................................
..........#....#....#..#.....#..............#.....
.............##.................#.................
...#..#.#................#..................#.....
........#.#.#....#......................#.........
..#................................#..#.......#...
#..........#..##.................#................
.#..........#..##...............##....#...........
..................##...........#.................#
..#..........#...........................#........
.......##...#.#...#.#...........###...............
.....#..#...#....#..#.....#................#......
..#........#......................................
..#.............##................#...#...........
............#....#..........#.#...................
.......#............#..#.....#.......##.......#...
............................#.....................
...#..#......#..................#.................
...............#......#...#...#...........#.......
......#...........................#...#.#..##.##..
.....#..........#..........................#......
...#...........##..#.........................#....
...#........#................#.............#....#.
.......##.......##.#......#................#......
.#........#........#...........#...............#..
..#............................#..#...............
...........................#.......#.....#........
.................#....##.....#..#..#....##..##....
.....................#.............#.....#........
..#..........................................#.#..
.........#..#.#..#...#................#..#.#......
..................#...#.#.......#.................
...#.........#.......#...###...........##...#.....
........................................#.........
......##......#............#.....#.......#..#.###.
#.....................#.......#.....#.............
...##..#..........#........#.....#.#......#.......
...#..#..#......#...#.............#.......#...#...

"""

    answer = AocDay22(data,silver_tests,gold_tests,argv)

    print(answer)
