...
screen = [[0]*xdim for _ in range(ydim)]

for line in data.split("\n"):
	mrect = rect.search(line)
	mrow = row.search(line)
	mcol = col.search(line)
	if mrect:
		(x,y) = [int(i) for i in mrect.group(1,2)]
		for yi in range(y):
			for xi in range(x):
				screen[yi][xi] = 1
	elif mrow:
		(y,num) = [int(i) for i in mrow.group(1,2)]
		screen[y] = screen[y][-num:]+screen[y][:-num]
	elif mcol:
		(x,num) = [int(i) for i in mcol.group(1,2)]
		tmp = [screen[y][x] for y in range(ydim)]
		tmp = tmp[-num:]+tmp[:-num]
		for y in range(ydim):
			screen[y][x] = tmp[y]

for y in range(ydim):
	print("".join(["#" if i else " " for i in screen[y]]))
print()

answer = sum([sum(screen[y]) for y in range(ydim)])
print(answer)
