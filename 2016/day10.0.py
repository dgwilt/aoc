...
data = '''bot 171 gives low to bot 4 and high to bot 84
...
maxbot = max([int(i) for i in re.compile(r'bot (\d+)').findall(data)])
maxout = max([int(i) for i in re.compile(r'output (\d+)').findall(data)])

bots = [Bot(i) for i in range(maxbot+1)]
outs = [Out(i) for i in range(maxout+1)]

give = re.compile(r'bot (\d+) gives low to (bot|output) (\d+) and high to (bot|output) (\d+)')

for line in data.split("\n"):
	mgive = give.search(line)
	if mgive:
		(b,low,high) = [int(i) for i in mgive.group(1,3,5)]
		(low_to,high_to) = mgive.group(2,4)
		l = bots[low] if low_to == "bot" else outs[low]
		h = bots[high] if high_to == "bot" else outs[high]
		bots[b].setup(l,h)

val = re.compile(r'value (\d+) goes to bot (\d+)')

for line in data.split("\n"):
	mval = val.search(line)
	if mval:
		(v,b) = [int(i) for i in mval.group(1,2)]
		bots[b].receive(v)
...
print(reduce(mul, [o.first() for o in outs[:3]], 1))
