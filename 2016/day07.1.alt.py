...
# This uses the newer regex library which you have to pip-install, which supports overlapped searches via a flag
import regex as re

aba_pattern = r'(\w)((?!\1)\w)\1'
square_pattern = r'\[(\w+)\]'
remove = re.compile(square_pattern)

answer = 0
for line in data.split("\n"):
    in_square = " ".join(re.findall(square_pattern, line))
    ab_in_square = set([(a,b) for (a,b) in re.findall(aba_pattern,in_square,overlapped=True)])
    
    out_square = remove.sub(" ",line)    
    ab_out_square = set([(a,b) for (b,a) in re.findall(aba_pattern,out_square,overlapped=True)])
        
    if len(ab_in_square.intersection(ab_out_square)) > 0:
        answer += 1
