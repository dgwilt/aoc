...
import re
import string
regex = re.compile('\A([a-z]+)(\d+)\[(\w+)\]\Z')

sid_sum = 0
for line in data.replace("-","").split("\n"):
	matches = re.match(regex,line)
	if matches is not None:
		(name,sid,chk) = matches.group(1,2,3)

		# Find how many of each letter there are in the name
		letter_counts = {l:name.count(l) for l in string.ascii_lowercase}

		ans = ""
		# Make a set of unique values of letter counts, then reverse-sort 
		# it so that we are starting with the largest count
		for c in sorted(list(set(letter_counts.values())),reverse=True):
			# Sort any letters alphabetically if there are draws
			ans += "".join(sorted([letter for letter,count in letter_counts.items() if count == c]))
			if len(ans) >= 5:
				# Stop when we have a long-enough answer
				break

		# Compare the first 5 letters with the checksum
		if str(ans[:5]) == chk:
			sid_sum += int(sid)
