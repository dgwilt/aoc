...
class AocDay13(AocDay):
    ...
    def num_tokens(data,offset):
        tokens = 0
        a, b = symbols('a b')
        for machine in data.split("\n\n"):
            xa,ya,xb,yb,xt,yt = [int(i) for i in findall(r'\d+',machine)]
            apress,bpress = next(iter(linsolve((Eq(xa*a + xb*b, xt+offset), 
                                                Eq(ya*a + yb*b, yt+offset)), (a, b))))
            if apress.is_integer and bpress.is_integer:
                tokens += 3*apress + bpress
        return tokens

    def run_silver(self,data):
        return AocDay13.num_tokens(data,0)
            
    def run_gold(self,data):
        return AocDay13.num_tokens(data,10000000000000)

if __name__ == "__main__":

    silver_tests = ["""Button A: X+94, Y+34
Button B: X+22, Y+67
Prize: X=8400, Y=5400
