...
class AocDay10(AocDay):
    ...
    def find_pipe(self,data):
        ...
        queue = [(start,0)]
        while queue:
            (x0,y0),steps = queue.pop(0)
            if steps > 0 and visited[(x0,y0)] >= steps:
                break
            
            visited[(x0,y0)] = steps

            for dx,dy in pipes[grid[y0][x0]]:
                x = x0 + dx
                y = y0 + dy
                if 0 <= x < xdim and 0 <= y <= ydim:
                    for dx2,dy2 in pipes[grid[y][x]]:
                        if (x+dx2,y+dy2) == (x0,y0) and (x,y) not in visited:
                            queue.append(((x,y),steps+1))
                
        return steps,visited,grid,xdim,ydim
    
    def run_silver(self,data):
        return self.find_pipe(data)[0]

    def run_gold(self,data):
        _,pipe,grid,xdim,ydim = self.find_pipe(data)

        # Clear out any non-pipe characters
        for x,y in product(range(xdim),range(ydim)):
            if (x,y) not in pipe:
                grid[y][x] = "."
        
        # The S square does not seem to affect the result thankfully, otherwise 
        # I'd have to add some code here to replace the S with the correct pipe symbol
        inside = 0
        for r in range(ydim):
            state = "O"
            for c in range(0,xdim):
                if state == "O":
                    match grid[r][c]:
                        case "|": state = "I"
                        case "L": state = "PLOI"
                    ...
                    match grid[r][c]:
                        case "J": state = state[-1] if state.startswith("PF") else state[-2]
                        case "7": state = state[-1] if state.startswith("PL") else state[-2]
                elif state == "I":
                    match grid[r][c]:
                        case ".": inside += 1
                        case "|": state = "O"
...
L7JLJL-JLJLJL--JLJ.L"""]
    ...
    data = """.L7F--|-77FLF7-F-FLL.FL7|7FL7-7.7-FJ7FJ-F.|7-.F|-|F-JFJFL-J-.F|.7F--77|77F7F|J77|-|7|-F-7.|JFL7-7.J---J.F7--.7F|--LJFL-J.FFFF.-L|---7777F7-|
