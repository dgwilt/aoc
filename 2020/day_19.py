...
class AocDay19(AocDay):
    ...
    def product(self,rules):
        return ''.join(self.rulepat(r) for r in rules)

    def ors(self,pats):
        return '|'.join(pat for pat in pats)

    def rulepat(self,r):
        rule = self.rulemap[r]
        return rule if isinstance(rule,str) else f"({self.ors([f'({self.product(o[:-1])})+' if o[-1] == r else f'({self.product(o)})' for o in rule])})"
    ...
    def rule0(self):
        return fr"\A{self.rulepat('0')}\Z"

    def setup(self,data):
        rules, messages = data.split('\n\n')
        LEAF = "ab"
        self.messages = messages.splitlines()
        self.rulemap = {rnum:rule.replace('"',"") if any(x in rule for x in LEAF) else [o.split() for o in rule.split(" | ")] for rnum,rule in self.parser(rules)}

    def run_silver(self,data):
        self.setup(data)            
        pat = compile(self.rule0())
        return sum(pat.search(m) != None for m in self.messages)
    ...
    def run_gold(self,data):
        ...
        self.rulemap['8'] = [['42'],['42','8']]
        self.rulemap['11'] = R11 # Need to handle rule 11 separately

        pat = self.rule0().replace(R11,fr"(({self.rulepat('42')}){{{NUM}}}({self.rulepat('31')}){{{NUM}}})")

        # a^nb^n is not a regular language, so have to go by trial and error. 5 is small enough for n
        pats = [compile(pat.replace(NUM,str(n))) for n in range(1,5)]
        return sum(any(p.search(m) for p in pats) for m in self.messages)

if __name__ == "__main__":

    data = """101: 126 91 | 59 129
106: 91 84 | 129 113
0: 8 11
84: 91 86 | 129 91
72: 44 129 | 69 91
