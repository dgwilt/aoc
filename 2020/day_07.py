...
class AocDay7(AocDay):
    ...
    def parser(self,data):
        for line in data.splitlines():
            ...
            outer = outer.strip()
            for i in inners.split(","):
                if i.strip().startswith("no"): continue
                b, inner = search(r'\s*(\d+)\s+(.*?)bag',i).group(1,2)
                yield outer, inner.strip(), int(b)

    def run_silver(self,data):
        G = nx.DiGraph((inner,outer,{'bags':b}) for outer, inner, b in self.parser(data))
        return len(nx.single_source_shortest_path(G,'shiny gold')) - 1
        
    def bags_from(self,G,outer):
        return 1 + sum(G.get_edge_data(outer,inner)['bags'] * self.bags_from(G,inner) for inner in G[outer])

    def run_gold(self,data):
        G = nx.DiGraph((outer,inner,{'bags':b}) for outer, inner, b in self.parser(data))
        return self.bags_from(G,'shiny gold') - 1

if __name__ == "__main__":

    data = """vibrant lavender bags contain 1 shiny coral bag, 4 dotted purple bags.
...
dull lime bags contain 1 dark teal bag."""
    ...
    silver_tests = ["""light red bags contain 1 bright white bag, 2 muted yellow bags.
...
dotted black bags contain no other bags.""",""""""]
    ...
    gold_tests = ["""shiny gold bags contain 2 dark red bags.
