...
class AocDay11(AocDay):
    ...
    def setup(self,data):
        seat_grid = [list(row) for row in data.splitlines()]
        rdim, cdim = len(seat_grid), len(seat_grid[0])
        self.deltas = [d for d in product([-1,0,1],[-1,0,1]) if d != (0,0)]
        self.swapper = {AocDay11.FULL:AocDay11.EMPTY, AocDay11.EMPTY:AocDay11.FULL}
        self.floorplan = {(r,c):seat_grid[r][c] for r,c in product(range(rdim),range(cdim))}
        self.seats = [k for k,v in self.floorplan.items() if v != AocDay11.GAP]

    def adjacent(self,s):
        r,c = s
        return [self.at(a) for a in product(range(r-1,r+2),range(c-1,c+2)) if a != s]

    def decide_flip(self,occupied,s,omin):
        return not occupied if self.at(s) == AocDay11.EMPTY else occupied >= omin

    def flip_silver(self,s):
        occupied = self.adjacent(s).count(AocDay11.FULL)
        return self.decide_flip(occupied,s,4)

    def at(self,s):
        return self.floorplan.get(s,None)

    def visible(self,s,d):
        v = add(s,d)
        while self.at(v) == AocDay11.GAP:
            v = add(v,d)
        return self.at(v)

    def flip_gold(self,s):
        occupied = [self.visible(s,d) for d in self.deltas].count(AocDay11.FULL)
        return self.decide_flip(occupied,s,5)

    def change(self,s):
        self.floorplan[s] = self.swapper[self.floorplan[s]]

    def run_seating(self,data,flip_fn):
        self.setup(data)
        while changes := [s for s in self.seats if flip_fn(s)]:
            for s in changes: self.change(s)
        return list(self.floorplan.values()).count(AocDay11.FULL)

    def run_silver(self,data):
        return self.run_seating(data,self.flip_silver)

    def run_gold(self,data):
        return self.run_seating(data,self.flip_gold)

if __name__ == "__main__":

    data = """LLLLLLLLL.LLLL.LLLLL.LLLLLLLLLL.LLLLLLLLLLLLLLLL.LLLLLL.LLLLLLLLLLLLLLLLLL.LLLLLLL.LLLLLLLL
