...
class AocDay4(AocDay):
    ...
    def __init__(self,*args):
        super().__init__(*args)
        self.required = 'byr iyr eyr hgt hcl ecl pid'.split()

    def parser(self,data):
       for passport in data.split("\n\n"):
            yield dict(field.split(":") for field in passport.split())

    def run_silver(self,data):
        return sum(all(r in passport for r in self.required) for passport in self.parser(data))
    
    def is_valid(self,passport):
        # Presence checks
        try: byr, iyr, eyr, hgt, hcl, ecl, pid = (passport[r] for r in self.required)
        except KeyError: return False

        # Type checks
        try: byr, iyr, eyr = (int(x) for x in (byr, iyr, eyr))
        ...
        return (1920 <= byr <= 2002
                and 2010 <= iyr <= 2020
                and 2020 <= eyr <= 2030
                and search(r'\A#[0-9a-f]{6}\Z',hcl) != None
                and ecl in 'amb blu brn gry grn hzl oth'
                and len(pid) == 9
                and ((hgt.endswith('in') and 59 <= int(hgt[:-2]) <= 76)
                    or (hgt.endswith('cm') and 150 <= int(hgt[:-2]) <= 193)))
        
    def run_gold(self,data):
        return sum(self.is_valid(passport) for passport in self.parser(data))
...
ecl:oth hcl:#602927 eyr:2025 iyr:2013 hgt:151cm byr:1992 pid:812583062"""
    ...
    silver_tests = ["""ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
iyr:2011 ecl:brn hgt:59in""",""""""]
    ...
    print(answer)
"""
Full version but it turns out that some is not required
return (byr.isdigit() and (1920 <= int(byr) <= 2002) 
        and iyr.isdigit() and (2010 <= int(iyr) <= 2020) 
        and eyr.isdigit() and (2020 <= int(eyr) <= 2030) 
        and match(r'#[0-9a-f]{6}\Z',hcl) != None
        and ecl in 'amb blu brn gry grn hzl oth'
        and pid.isdigit() and len(pid) == 9
        and match(r'\d+(in|cm)\Z',hgt) != None
        and ((hgt.endswith('in') and 59 <= int(hgt[:-2]) <= 76)
            or (hgt.endswith('cm') and 150 <= int(hgt[:-2]) <= 193)))
"""
