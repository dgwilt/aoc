...
class AocDay2(AocDay):
    ...
    def __init__(self,*args):
        super().__init__(*args)
        self.parser_setup(types=(int,int,str,str), pat=r'(\d+)-(\d+)\s+(\w):\s+(\w+)')

    def run_silver(self,data):
        return sum(mn <= pwd.count(letter) <= mx for mn, mx, letter, pwd in self.parser(data))
        
    def run_gold(self,data):
        return sum((pwd[mn-1],pwd[mx-1]).count(letter) == 1 for mn, mx, letter, pwd in self.parser(data))

if __name__ == "__main__":

    data = """8-9 x: xxxxxxxrk
5-8 f: fxffmfff
2-5 c: ccspwpc
1-4 g: gggg
9-10 l: lllllllllk
...
7-11 g: xzgnggggrggrg"""
    ...
    silver_tests = ["""1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc""",""""""]

    gold_tests = ["""1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc""",""""""]
